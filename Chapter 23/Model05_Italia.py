...
#Detected_Model.Wavelets += [Hubbert( x0=119.1, s0=0.390, s1=0.200, peak=5800)]
Detected_Model.Wavelets += [Hubbert( x0=110.5, s0=0.80, s1=0.65, peak=5100)]
#Detected_Model.Wavelets += [Hubbert( x0=105.5, s0=0.30, s1=0.1, peak=1800)]
...
plt.savefig( "./Graphs/figure_04_Italy.png")
...
##time = np.linspace(0, 183, 184)
##
##Model_Exp = np.exp(0.16*(time-44))
##Model_Exp_Death = np.exp(0.16*(time-60))
##
##Model = Linear_Combo()
##Model.Wavelets += [Sigmoid( x0=117.000, s0=0.17, left=0, right=nRight)]
##Model1 = Linear_Combo()
##Model1.Wavelets += [Sigmoid( x0=110.000, s0=0.255, s1=0.16, left=0, right=nRight/detection)]
##
...
##Model_Deaths.Wavelets += [Sigmoid( x0=118, s0=0.17, left=0, right=16000)]
...
##ax3 = plt.subplot(gs[2])
##
##ax1.plot( Day[0:limit], WHO_Cases[0:limit]*norm, 'o', lw=5, color='b', label="Случаи-ВОЗ")
##ax1.plot( time, Model_Total_Cases*norm, '--', color='g', lw=2, label="Инфицировано (мoдель)")
##ax1.plot( time, Model_WHO_Cases*norm, '-', color='g', lw=3, label="Выявлено (мoдель)")
##ax1.plot( time, Model_Exp*norm, '-.', color='g', lw=3, label="Экспонента бледнолисых")
##ax1.annotate('Первые два случая 31 января',
##                xy=(62,10*norm),xytext=(5,30000*norm),
##                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.5),
##                bbox=dict(pad=1, facecolor="none", edgecolor="none"))
##ax1.set_xlim( xlimits)
##ax1.set_ylim( 0, 500000*norm)
##ax1.set_ylabel("тысяч инфицированных")
##ax1.grid(True)
##ax1.legend(loc=0)
##
##ax2.plot( Day[0:limit], WHO_Deaths[0:limit], 'o', lw=5, color='r', label="Смерти-ВОЗ")
##ax2.plot( time, Model_Deaths[:184], '-', color='k', lw=2, label="Смерти (модель)")
##ax2.plot( time, Model_Exp_Death, '-.', color='k', lw=3, label="Расчёт смертей у бледнолисых")
##ax2.annotate('Ожидаемый cIFR={:.1f}%'.format(Model_Deaths[-1]*100/Model_WHO_Cases[-1]),
##                xy=(175,Model_Deaths[-1]),xytext=(130,Model_Deaths[-1]*1.1),
##                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.5),
##                bbox=dict(pad=1, facecolor="none", edgecolor="none"))
##ax2.annotate('Первые две смерти 23 февраля',
##                xy=(85,10),xytext=(5,5000),
##                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.5),
##                bbox=dict(pad=1, facecolor="none", edgecolor="none"))
##ax2.set_xlim( xlimits)
##ax2.set_ylim( 0, 25000)
##ax2.set_ylabel("смертей")
##ax2.grid(True)
##ax2.legend(loc=0)
##
##ax3.plot( time, Model_Infection_Rate, "--", lw=2, color='g', label="Заразились (модель)")
##ax3.plot( time, Model_Detection_Rate, "-", lw=3, color='g', label="Выявлено (модель)")
##ax3.plot( time, Model_Death_Rate[:184]*10, "-", lw=3, color='k', label="Смертей (x10)")
##ax3.annotate('Матч "Лиги Чемпионов" в Милане 19 февраля',
##                xy=(81,20),xytext=(5,3000),
##                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.5),
##                bbox=dict(pad=1, facecolor="none", edgecolor="none"))
##ax3.set_xlim( xlimits)
