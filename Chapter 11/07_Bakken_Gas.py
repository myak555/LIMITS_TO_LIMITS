...
if metric:
    ...
    ax1.errorbar( Year[:-24], Actual[:-24], yerr=Actual[:-24]*.05, fmt='o', color="k", label="Включая попутный газ ({:.2f} 10¹² м³)".format(np.sum(Actual[:-24])/1000))
    ax1.errorbar( YC, PC, yerr=PC*.05, fmt='o', color="r", label="Только из газовых скважин")
    ax1.set_ylim( 0, 60)
    ax1.set_ylabel("Млрд м³ в год")
    ax1.annotate("Пик добычи вроде бы в марте 2015", xy=(2015, 16.6), xytext=(2020, 25), arrowprops=dict(facecolor='black', shrink=0.05))
else:
