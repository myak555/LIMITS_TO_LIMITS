...
class Simple_Calibration:
    def __init__( self, year0=1890):
        ...
        self.efficiency = 1.0
        self.investment = 0.07
        self.depreciation = 0.04
        self.FOC_Model = Sigmoid( 0.5, 10, -0.90, -0.01, 1)
        #self.FOC_Model.Plot(0, 1)
        self.Compute_Next_Year()
        return
    def Compute_Next_Year(self):
        self.Year += 1
        alpha = self.FOC_Model.Compute(self.Q / self.Q0)
        production = self.C * self.productivity * alpha
        self.M = (1-self.investment) * production
        self.C += self.investment * production - self.depreciation * self.C
        self.dQ = self.C / self.efficiency
        self.Q -= self.dQ
        if self.Q < 0.0: self.Q = 0.0 
        #print(self.Year, production)
        return
    def Solve(self, nyears):
        self.Time = np.linspace( self.Year, self.Year+nyears, nyears+1)
        self.Capital = np.zeros( nyears+1)
        self.Resources = np.zeros( nyears+1)
        self.Extraction = np.zeros( nyears+1)
        self.Goods = np.zeros( nyears+1)
        for i, t in enumerate(self.Time):
            self.Capital[i] = self.C
            self.Resources[i] = self.Q
            self.Extraction[i] = self.dQ
            self.Goods[i] = self.M
            self.Compute_Next_Year()
        return
...
if __name__ == "__main__":
    sc = Simple_Calibration()
    sc.Solve( 210)
    plt.plot(sc.Time, sc.Goods, color='b')
    plt.plot(sc.Time, sc.Extraction, color='r')
...
##    """
##    def __init__( self, year0=1890, initial=100, lifetime=25, output_ratio=0.69, consumption_ratio=0.77, verbose=False):
##        self.Total = initial
...
##        self.Indicated_Per_Capita_Min = 0.05
##        self.Indicated_Per_Capita_Max = 0.50
...
##        #ax3.plot( self.W3_mod.Population[:129]/1000, self.R0.Calibration_Carbon/1000, ".", lw=1, color="b", label="Carbon vs Population")
##        #ax3.plot( self.W3_mod.Time[:129], self.R0.Calibration_Carbon/self.W3_mod.Population[:129], ".", lw=1, color="b", label="Carbon vs Population")
##        #ax3.plot( self.W3_mod.Time[:129], self.R0.Calibration_Carbon/self.W3_mod.Goods[:129], ".", lw=1, color="b", label="Carbon vs Goods")
##        #ax3.plot( self.W3_mod.Time[:129], self.R0.Calibration_Carbon/self.W3_mod.Services[:129], "--", lw=3, color="m", label="Carbon vs Services")
##        #ax3.plot( self.W3_mod.Time[:129], self.R0.Calibration_Carbon/self.W3_mod.Food[:129], "--", lw=3, color="g", label="Carbon vs Food")
##        ax3.plot( self.W3_mod.Time[:129],
##                  self.R0.Calibration_Carbon/self.Industrial_Capital_Actual[:129],
##                  "-", lw=3, color="b", label="Carbon vs Industrial capital")
##        ax3.plot( self.W3_mod.Time[:129],
##                  self.R0.Calibration_Carbon/self.Agriculture_Capital_Actual[:129],
##                  "-", lw=3, color="g", label="Carbon vs Agriculture capital")
##        ax3.plot( self.W3_mod.Time[:129],
##                  self.R0.Calibration_Carbon/self.Service_Capital_Actual[:129],
##                  "-", lw=3, color="m", label="Carbon vs Service capital")
##
