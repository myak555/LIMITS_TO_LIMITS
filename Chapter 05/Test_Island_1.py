...
class Island_Population_1:
    ...
    def dP_dt( self, t):
        ...
        return tmp
    def dQ_dt( self, t):
        tmp = -self.D * self.Q # естественная убыль запасов
        tmp -= self.P          # убыль запасов на потребление
        tmp += self.O          # текущий урожай
        return tmp
    def dO_dt( self, t):
        tmp = max ( self.F * self.J, 0.01)
...
productivity = 0.001
P1 = Island_Population_1( 500, 500, 1000, 500000, 25, 5, t2_q, 1.0, productivity)
P1.Solve( T)
b1 = np.log(2) / 25
...
plt.ylabel("единиц")
plt.ylim( 0, 10000)
plt.title( "Остров Мальтуса #1 - Примитивное племя")
plt.grid(True)
plt.legend(loc=0)
