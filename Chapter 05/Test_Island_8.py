...
class Island_Population_5:
    def __init__( self, P0, Q0, O0, F0, T2_P, T2_O, T2_F, d_Q, j_O):
        ...
        return
    def is_Drought( self, t):
        tt = int(t)
        if (tt%10) == 0: return True
        return False
    def dP_dt( self, t):
        tmp = max ( self.Q, 0.01)
...
productivity = 0.001
P1 = Island_Population_5( 500, 500, 1000, 500000, 25, 5, t2_q, 0.1, productivity)
P1.Solve( T)
b1 = np.log(2) / 55
...
plt.plot( T, Exponent1, "--", lw=1, label="Экспоненциальный рост Т2=55 лет")
plt.xlabel("Годы")
plt.xlim( 0, 1000)
plt.ylabel("единиц")
plt.ylim( 0, 15000)
...
plt.grid(True)
plt.legend(loc=0)
plt.savefig( ".\\Graphs\\figure_05_12.png")
fig.show()
