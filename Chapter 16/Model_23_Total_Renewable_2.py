...
class Energy_Model_7:
    def __init__( self, Resource, Population, ERoEI, q0, URR=1400e3, staple_level = 1.5):
        ...
        self.q0 = q0
        self.ERoEI = ERoEI
        self.ERoEI_Renewable = 5 #10
        self.ERoEI_Nuclear = 5
        ...
        self.Deprivation = 1
        self.DeprivationRN = Sigmoid( 5000,0.05,1.02,1) 
        ...
        self.Nuclear_Decline = Sigmoid(2025,0.5,1,1) # Effect of nuclear moratorium (set Right=1 to switch off)
        #self.Nuclear_Decline.Plot(1900,2100)
        return
    ...
    def _Add_Nuclear(self, t0):
        ...
        self.Production_Nuclear *= self.Nuclear_Decline.GetVector(t0)
        for i in range( len( t0)):
            t = t0[i]
            if t < 1954:
...
plt.savefig( ".\\Graphs\\figure_16_23.png")
fig.show()
