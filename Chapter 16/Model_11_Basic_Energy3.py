        ...
        for i in range( 1,len(t0)-len(self.Return_Function)):
            ...
            if deltaQ < 0:
                ...
                if t0[i] <= 2017: # stick to actual
                    ...
                    self.UsefulProduction[i-1] = self.ProductionCalibrated[i-1] * (1-1/self.ERoEI)
...
P = Population()
R = Resources()
E = Energy_Model_3( R, P.UN_Medium, 40)
E.Solve( Year)
...
fig.suptitle( 'Простейшая модель энергетики (цепочки Маркова)', fontsize=22)
