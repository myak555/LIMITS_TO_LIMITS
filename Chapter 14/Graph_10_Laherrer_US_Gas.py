...
plt.plot( Year[170:], Conventional[170:], "-", lw=2, color="g", label="Газ из проницаемых пород, URR={:.1f} млрд м³".format(np.sum(Conventional)/1000))
plt.plot( Year[150:], Unconventional[150:], "--", lw=2, color="r", label="Газ из ТрИЗ, URR={:.1f} трлн м³".format(np.sum(Unconventional)/1000))
plt.plot( Year, Total, "-", lw=2, color="r", label="Laherrer (2014), URR={:.1f} трлн м³".format(np.sum(Total)/1000))
plt.plot( Prediction_T, Prediction_Total_Hughes, ".", lw=1, color="r", label="Hughes (2016), URR={:.1f} трлн м³".format(np.sum(Prediction_Total_Hughes)/1000+34.2))
plt.errorbar( Year_US2, Gas_US2, yerr=Gas_US2*0.05, fmt='.', color="m", label="BP с 1970 по 2017, UR={:.1f} трлн м³".format(np.sum(Gas_US2)/1000))
plt.annotate("Добыто с 1821 года: {:.1f} трлн м³".format( np.sum(Total[0:217])/1000), xy=(2017, 763), xytext=(2030, 800), arrowprops=dict(facecolor='black', shrink=0.05))
plt.annotate('Конец "сланцевой революции"', xy=(2021, 594), xytext=(2030, 650), arrowprops=dict(facecolor='black', shrink=0.05))

plt.xlim( 1900, 2100)
plt.ylim( 0, 900)
plt.xlabel( "Год")
plt.ylabel( "миллиардов м³ в год")
plt.title( 'Добыча природного газа в США 1900-2017 гг')
plt.grid(True)
plt.legend(loc=0)
