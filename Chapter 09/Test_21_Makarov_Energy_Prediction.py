...
img = imread( cbook.get_sample_data( os.getcwd() + '/Makarov_Energy_Projection.jpg'))
plt.imshow(img, zorder=0, extent=[1853, 2037, -4.35, 16.35 ],  interpolation='nearest', aspect='auto')
plt.plot( [1860, 1860], [-1, 19], "--", lw=1, color="m")
plt.plot( [2030, 2030], [-1, 19], "--", lw=1, color="m")
plt.plot( [1800, 2100], [0, 0], "--", lw=1, color="m")
plt.plot( [1800, 2100], [16, 16], "--", lw=1, color="m")

plt.plot( T, Biomass/1000, "--", lw=1, color="y", label="Биомасса и мусор (РАН, с графика)")
...
plt.plot( [2010,2010], [0,20], "-.", lw=1, color="k")

plt.xlabel("Годы")
plt.xlim( 1850, 2050)
plt.ylabel("миллиарды тонн")
plt.ylim( -5, 22)
plt.title( 'Энергетическое предсказание А.Макарова против статистики "ВР"')
plt.grid(True)
plt.legend(loc=2)
plt.savefig( "./Graphs/figure_09_21.png")
