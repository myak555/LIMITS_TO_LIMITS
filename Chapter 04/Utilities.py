...
class Control_Curve:
    ...
    def Compute_Clipped(self, x, min_y=0.0, max_y=1.0):
        """
    ...
    def GetVector(self, x):
        ...
        for i, vx in enumerate(x):
            y[i] = self.Compute(vx)
        return y
    def GetVector_Clipped(self, x, min_y=0.0, max_y=1.0):
        """
...
if __name__ == "__main__":
    ...
    F1 = Sigmoid(-1)
    F1.Plot(coplot_x[0], coplot_x[-1],"./Graphs/Test01_Sigmoid",
            coplot_x, coplot_y, no_screen = True)
    F2 = Bathtub()
    F2.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test02_Bathtub",
            coplot_x, F1.GetVector(coplot_x), no_screen = True)
    F3 = Hubbert()
    F3.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test03_Hubbert",
            no_screen = True)
    F4 = GenHubbert()
    F4.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test04_GenHubbert",
            no_screen = True)
    F5 = Gauss()
    F5.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test05_Gauss",
            no_screen = True)
    F6 = Kapitsa()
    F6.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test06_Kapitsa",
            no_screen = True)
    F7 = KapitsaIntegral()
    F7.Plot(1800, 2200, "./Graphs/Test07_Kapitsa_Integral",
            no_screen = True)
    F8 = Weibull(x0=1968, b=0.015, k=2.2, peak=100)
    F8.Plot(1900, 2100, "./Graphs/Test08_Weibull",
            no_screen = True)
    F9 = Gompertz()
    F9.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test09_Gompertz",
            no_screen = True)
    F10 = GompertzIntegral()
    F10.Plot(coplot_x[0], coplot_x[-1], "./Graphs/Test10_Gompertz_Integral",
             no_screen = True)
